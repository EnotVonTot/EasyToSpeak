...
class ItemAdapter (val con:Context, val list2:ArrayList<Items>):RecyclerView.Adapter<ItemAdapter.Link>() {
    class Link(itemView: View):RecyclerView.ViewHolder(itemView){
        val im_di:ImageView = itemView.findViewById(androidx.constraintlayout.widget.R.id.image)
        val text_di:TextView = itemView.findViewById(androidx.core.R.id.title)
    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemAdapter.Link {
        val root = LayoutInflater.from(con).inflate(R.layout.item_layout, parent, false)
        return Link(root)
    }
